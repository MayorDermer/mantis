...
#include <mantis/detail/driver_impl/virtual/params/virtual_msdr_params.h>
...
#undef PARAM
         ...
         */
        [[nodiscard]] std::string get_find_args() const;
        ...
        /**
         * @brief function used to compare params for "equality" will only invalidate the comparison if the specific
         * params has a field differing from the generic params that is not null for example, if the specific params has
         * product set to X310, and the generic params has no product set, the result will be false. However, in the
         * reverse case the result will be true. The goal is to find any sdr that matches the "constraints" of the
         * specific params
         ...
         */
        static msdr_params from_uhd_device_addr(uhd::device_addr_t address, const msdr_params& original);
         ...
         */
        static errors::error_code from_str(const std::string& args, params::msdr_params& new_params,
