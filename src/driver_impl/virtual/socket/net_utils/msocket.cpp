...
mantis::net::msocket::msocket(params::msocket_params _sock_params)
        : sock_params(std::move(_sock_params)) {
    this->sockfd = socket(this->sock_params.domain, this->sock_params.type,
                          this->sock_params.protocol);
    if (this->sockfd < 0) {
        throw mantis::runtime_error(
                "Failed to create socket with: " +
                std::to_string(this->sock_params.domain) + ", " +
...
mantis::errors::error_code mantis::net::msocket::connect() {
    ...
    if (inet_pton(this->sock_params.domain, this->sock_params.host.c_str(),
                  &this->sockaddr.sin_addr.s_addr) <= 0) {
        perror("socket address conversion failed");
        throw mantis::runtime_error(
                "Failed to convert host: " +
                this->sock_params.host);
    }

    socklen_t addrlen = sizeof(this->sockaddr);
    if (::connect(this->sockfd, (struct sockaddr *) &this->sockaddr,
                  addrlen) < 0) {
...
mantis::errors::error_code mantis::net::msocket::send(const char *data,
                                                      size_t size) {
    size_t total_bytes_sent{0};
    ssize_t bytes_sent{0};
    while (total_bytes_sent < size) {
        int bytes_to_send =
                this->sock_params.type == SOCK_STREAM ? size : (size - total_bytes_sent) > MAX_UDP_SIZE ? MAX_UDP_SIZE :
...
mantis::errors::error_code mantis::net::msocket::recv(char *data, size_t size) {
    ...
    while (bytes_needed > 0) {
        ...
        ssize_t bytes_received = ::recv(this->client_fd, data + (size - bytes_needed), bytes_to_recv,
                                        this->sock_params.flags);


        if (bytes_received <= 0 && bytes_needed == size) {
